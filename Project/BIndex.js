...
const table_204 = document.getElementById('body_204');
const table_205 = document.getElementById('body_205');
var status1 = 2;
var status2 = '全部';
...
function search_204() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            var data = JSON.parse(xhr.responseText);
            //刷新
            while (table_204.firstChild) {
                table_204.removeChild(table_204.firstChild);
            }
            //返回查询结果
            for (var i = 0; i < data.length; ++i) {
                const userid = data[i].userid;
                const times = data[i].times;
                const lasttime = data[i].lasttime;
                const tr = document.createElement('tr');
                const c4_1 = document.createElement('td');
                c4_1.textContent = userid;
                tr.appendChild(c4_1);
                const c4_2 = document.createElement('td');
                c4_2.textContent = times;
                tr.appendChild(c4_2);
                const c4_3 = document.createElement('td');
                c4_3.textContent = lasttime;
                tr.appendChild(c4_3);
                table_204.appendChild(tr);
            }
        }
    }
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    var user_204 = document.getElementById('usersearch').value;
    var formdata = "customer=" + user_204 + "&formtype=userman";
    xhr.send(formdata);
}
search_204();

document.getElementById('search_204').addEventListener('click', function () {
    search_204();
})
...
function search_205() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            var data = JSON.parse(xhr.responseText);
            //刷新
            while (table_205.firstChild) {
                table_205.removeChild(table_205.firstChild);
            }
            //返回查询结果
            for (var i = 0; i < data.length; ++i) {
                const gid = data[i].gid;
                ...
                const op = data[i].op;
                const optime = data[i].optime;
                const tr = document.createElement('tr');
                const c5_1 = document.createElement('td');
                c5_1.textContent = uid;
                tr.appendChild(c5_1);
                const c5_2 = document.createElement('td');
                c5_2.textContent = gid;
                tr.appendChild(c5_2);
                const c5_3 = document.createElement('td');
                if (op == 0)
                    c5_3.textContent = '浏览';
                else
                    c5_3.textContent = '购买';
                tr.appendChild(c5_3);
                const c5_4 = document.createElement('td');
                c5_4.textContent = optime;
                tr.appendChild(c5_4);
                table_205.appendChild(tr);
            }
        }
    }
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    var date_205 = document.getElementById('logdate').value;
    var formdata = "from=" + date_205 + " 00:00:00" + "&to=" + date_205 + " 23:59:59" + "&formtype=bloginfo";
    xhr.send(formdata);
}
search_205();

document.getElementById('search_205').addEventListener('click', function () {
    search_205();
})